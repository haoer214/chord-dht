...
public class NodeDHT implements Runnable //extends UnicastRemoteObject implements NodeDHTInterface
{
    ...
    // 网络中现存节点
//    private static List<Node> existNode;
    ...
    public static void main(String args[]) throws Exception
    {
        ...
        if (args.length < 3)
        {
            ...
            System.out.println("         *** [LocalPortNumber] = is the port number which the Node will be listening waiting for connections.");
        ...
        if (initInfo.equals("NACK")) {
            ...
            System.out.println("Connection to the SuperNode established successfully");
        ...
        // 节点退出的线程
        Runnable exitRunnable = new NodeDHT(temp,-1);
    ...
    public void beforeExit(){
        ...
        // 将后继节点的前驱节点设置为当前节点的前驱，并告知后继节点退出节点ID，通过后继节点获取网络中现存节点，并更新它们的路由表
        String lastRequest = "nodeExit/" + pred.getID() + "/" + pred.getIP() + "/" + pred.getPort() + "/" + me.getID();
        try {
            makeConnection(finger[1].getSuccessor().getIP(),finger[1].getSuccessor().getPort(),lastRequest);
        } catch (Exception e) {
            e.printStackTrace();
        }

        // 通过后继节点更新其他节点路由表(已合并)
        // String lastRequest2 = "updateOthers/" ;
        // try {
        //     makeConnection(finger[1].getSuccessor().getIP(),finger[1].getSuccessor().getPort(),lastRequest2);
        // } catch (Exception e) {
        //     e.printStackTrace();
        // }
    ...
    public void run() {
        ...
        if (this.ID == -1) {
            ...
            try { 
                service.finishJoining(me.getID()); // 释放同步锁
            } catch (Exception e) {}
        }
        else {
            try {
    ...
    public static String considerInput(String received) throws Exception {
        ...
        // 将前驱节点设置为退出节点的前驱，删除退出节点信息，获取网络中现存节点，并更新它们的路由表
        else if (tokens[0].equals("nodeExit")) {
            Node newNode = new Node(Integer.parseInt(tokens[1]),tokens[2],tokens[3]);
            setPredecessor(newNode);
            System.out.println("My new predecessor : " + pred.getID());

            // 必须使用数组（不能用List），否则不能进行序列化传输，无法得到返回值
            Node[] existNode = service.setNodeList(Integer.parseInt(tokens[4]));

            System.out.println("Exist Node num : " + existNode.length);
            System.out.println("Exit Node ID : " + Integer.parseInt(tokens[4]));
            update_others(existNode,Integer.parseInt(tokens[4]));
            System.out.println("Updated all other nodes Again!");
        }
        ...
        else if (tokens[0].equals("updateExitNodeFing")) {
            Node newNode = new Node(Integer.parseInt(tokens[1]),tokens[2],tokens[3]);
            update_existNode_finger_table(newNode,Integer.parseInt(tokens[4]));
            System.out.println("Because node " + Integer.parseInt(tokens[4]) + " exit, updated my fingers again!");
            outResponse = "update exist node fingers successfully";
        }
    ...
    // 再次更新其他节点路由表时，新增一个exitID形参，表示退出节点的ID
    public static void update_others(Node[] existNode, int exitID) throws Exception{
        for (Node node : existNode){
            String request = "updateExitNodeFing/" + me.getID() + "/" + me.getIP() + "/" + me.getPort() + "/" + exitID;
            makeConnection(node.getIP(), node.getPort(), request);
        }
    }
    ...
    public static void update_existNode_finger_table(Node s, int exitID){
        for(int i = 1; i <= m; i++){
